...
parser.add_argument('--resume', default='./Universal/checkpoint_19.tar', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')#./Universal/checkpoint_20.tar
...
def cross_entropy_loss(out1,out2,out3,label,mlabel,clabel):
    ...
    loss = loss3#0.5 * loss1 + 0.3 * loss2 + 0.2* loss3
...
def main():
    ...
    train_num = load_file_list(args.load_path)
    test_num = load_test_list(args.load_path)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # prec1 = validate_back(test_class_length,model, criterion, optimizer, epoch)
...
def train(train_num, model, criterion, optimizer, epoch):
    ...
    iters = train_num//args.batch_size//10
...
def validate(test_num, model, criterion):
    ...
    batch_time = AverageMeter()
    losses = AverageMeter()
    top = init_AverageMeter(3)

    # switch to evaluate mode
    model.eval()

    end = time.time()
    iters = 200#test_num//args.batch_size
    for iter in range(iters):
        # measure data loading time
        ...
        losses.update(loss.item(), input.size(0))
        top[0].update(prec1.item(), input.size(0))
        top[1].update(prec2.item(), input.size(0))
        top[2].update(prec3.item(), input.size(0))
    ...
    with open("record.txt", "a+") as f:
        f.write("\t" + 'blabel: * Prec@1 {top1.avg:.3f}'.format(top1=top[0]))
        f.write("\t" + 'mlabel: * Prec@1 {top1.avg:.3f}'.format(top1=top[1]))
        f.write("\t" + 'blabel: * Prec@1 {top1.avg:.3f}'.format(top1=top[2]))
        f.write("\n")
    return prec1
